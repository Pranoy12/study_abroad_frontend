...
async def update_interaction_history(session_service, app_name, user_id, session_id, entry):
    ...
    try:
        ...
        # Initialize interaction history if not present
...
async def process_agent_response(event, runner, user_id, session_id):
...
async def call_agent_async(runner, user_id, session_id, query):
    ...
    try:
        async for event in runner.run_async(
            ...
            response = await process_agent_response(event, runner, user_id, session_id)
            if response:
